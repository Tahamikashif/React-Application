       ...
       <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-7">
            {products.map((item,i) => (
              ...
              <div className="w-[300px] ml-[30px] rounded-lg mt-[20px] h-[500px] hover:scale-105 duration-500 transition-transform bg-gray-300" key={i}>
                ...
                <p className="p-2">Name:{item.title}</p> 
                 <p className="p-2">Desc:{item.message}</p> 
                  <p className="p-2">Stock:{item.stock}</p>
                   <p className="p-2">Price: Rs {item.price}</p>
                    <p className="p-2">dis: {item.dis} %</p>   
