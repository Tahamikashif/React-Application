...
const Navbar = () => {
  ...
  return (
    <div className="mt-[25px] ml-[18px] flex items-center gap-4 relative">
      ...
      <div className="flex items-center bg-gray-200 rounded-lg px-2 w-[400px] relative">
        <input
          ...
          value={searchTerm}
          onChange={(e) => { setSearchTerm(e.target.value); setShowSuggestion(true); }}
          onFocus={() => { if (searchTerm.trim()) setShowSuggestion(true); }}
        />

        <IoSearch className="text-[18px]" />

        {showSuggestion && searchTerm.trim() !== "" && (
          <div className="absolute top-[40px] left-0 z-50 w-[400px] max-h-[400px] overflow-y-auto bg-white shadow-lg rounded-lg">
            {FilterProduct.length > 0 ? (
              FilterProduct.map((val) => (
                <Link
                  to={`/products/${val._id || val.id}`} // ✅ backend key safe check
                  key={val._id || val.id}
              ...
              <p className="text-center p-4 text-gray-500">No matching products found.</p>
            )}
          </div>
        )}
      </div>
